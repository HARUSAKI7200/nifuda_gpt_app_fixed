...
class HomePage extends ConsumerWidget {
      ...
      children: [
        ...
        _buildActionButton(
          ...
          onPressed: () async {
             ...
             if (state.selectedCompany != '(未選択)') {
                // プリセットかDBから検索
                final preset = MatchingProfileRegistry.availableProfiles.where((p) => p.label == state.selectedCompany).firstOrNull;
                if (preset != null) {
                  profile = preset;
                } else {
                  final maskProfile = state.maskProfiles.where((p) => p.profileName == state.selectedCompany).firstOrNull;
                  if (maskProfile != null) {
                    profile = MatchingProfileRegistry.fromDbProfile(maskProfile);
                  }
                }
             }
  ...
  Widget _buildDropdownSelector({
    ...
    required IconData icon, // ★ icon引数を追加
    required MaterialColor color,
    required Function(String?)? onChanged,
  }) {
    return Container(
      height: 56, // 高さを少し広げて見やすく
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          onChanged: onChanged,
          icon: Icon(Icons.arrow_drop_down_circle, color: color[700]),
          isDense: true,
          isExpanded: true,
        ),
      ),
