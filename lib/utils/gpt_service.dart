...
String _buildNifudaPrompt(List<String> fields) {
  final fieldsStr = fields.map((f) => "- $f").join('\n');
  return '''
あなたは、かすれたり不鮮明な荷札の画像を完璧に文字起こしする、データ入力の超専門家です。
あなたの使命は、一文字のミスもなく、全ての文字を正確にJSON形式で出力することです。以下の思考プロセスとルールを厳守してください。
...
Future<Map<String, dynamic>?> sendImageToGPT(
  ...
  final List<String> targetFields = nifudaFields ?? MatchingProfileRegistry.defaultNifudaFields;
  final String prompt = _buildNifudaPrompt(targetFields);
